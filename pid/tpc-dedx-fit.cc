...
void tpc_dedx_fit(const char* filename = "/home/had/haein/data/JPARC2025Nov_root/gain_calib_260616/run02602_DstTPCHelixTracking.root",
                        const char* histname = "PID_dEdx_vs_SignedMom",
                        const char* species_c = "pi",
                        double pmin = 0.10,
                        double pmax = 1.20,
                        double pstep = 0.05,
                        ...
                        const char* outname = "dedx_sigma_fit.root")
{
  ...
  double mass = DedxFit::Mass(species);
  ...
  for (double p = pmin; p < pmax; p += pstep) {
    ...
    if (ipoint < 20) {
      ...
      hy->Draw();
      gaus->Draw("same");
      //cfit->SaveAs(Form("dedx_fit_%s_%03d.png", species.c_str(), ipoint));
    }
  ...
  //c1->SaveAs(Form("dedx_mean_%s.png", species.c_str()));
  ...
  //c2->SaveAs(Form("dedx_sigma_%s.png", species.c_str()));
