...
void htofcalib_padgain(int runnumber){
  ...
  string dir = "/gpfs/group/had/sks/Users/haein/data/JPARC2025Nov_root/after_gain_calib_260523_0";
  TFile *file = new TFile(Form("%s/run0%d_DstTPCHelixTracking.root",dir.c_str(),runnumber));
  TTree *tree = (TTree*)file->Get("tpc");
  string outpdf = Form("htofcalib-padgain-run0%d-260523_allpad.pdf", runnumber);
  ...
  TFile *f = new TFile(Form("htofcalib-padgain-run0%d-260523_allpad.root",runnumber),"RECREATE");
